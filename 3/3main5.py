...
def broadcast_hash_links() -> None:
    ...
    for chat_id in tele_open_list:
        ...
        # build a row of URL buttons
        inline_rows = []
        for key in ("sub_1", "sub_2", "sub_3"):
            val = subs.get(key)
            if val is None:
                continue
            safe_sub = str(val).replace(".", "d")          # keep deep-link safe
            token    = f"{base_hash}_{safe_sub}"
            url      = f"https://t.me/{BOT_USERNAME}?start={token}"
            inline_rows.append({"text": str(val), "url": url})

        if not inline_rows:
            continue  # nothing to send for this channel

        reply_markup = {"inline_keyboard": [inline_rows]}  # one row, many buttons
        ...
        try:
            ...
            msg_id = r.json()["result"]["message_id"]
            # auto-delete after 60 s (reuse your existing TTL logic)
            del_url = f"https://api.telegram.org/bot{BOT_TOKEN}/deleteMessage"
            asyncio.get_event_loop().call_later(
                60,
                lambda: requests.post(
