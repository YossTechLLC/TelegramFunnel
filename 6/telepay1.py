...
# Utility: encode & sign success url parameters
def build_signed_success_url(tele_open_id, closed_channel_id, signing_key, base_url="https://us-central1-telepay-459221.cloudfunctions.net/success_inv"):
    ...
    params = {
        "tele_open_id": str(tele_open_id),
        "closed_channel_id": str(closed_channel_id),
        "ts": str(timestamp),
    }
    # sort for consistent signature, encode as query string
    ...
    # base64-url-encode the query string for opacity
    payload = urlencode(sorted(params.items()))
    payload_b64 = base64.urlsafe_b64encode(payload.encode()).decode().rstrip("=")
    return f"{base_url}?payload={payload_b64}"
