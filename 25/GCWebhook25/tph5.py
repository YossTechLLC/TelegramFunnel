...
def trigger_eth_payment_splitting(user_id: int, client_wallet_address: str, subscription_price: str) -> None:
    ...
    try:
        ...
        tps2_webhook_url = os.getenv("TPS2_WEBHOOK_URL")
        if not tps2_webhook_url:
            print("⚠️ [WARNING] TPS2_WEBHOOK_URL not configured - skipping ETH payment splitting")
            return
        ...
        # Prepare payload for tps2.py webhook
        payload = {
        ...
        print(f"❌ [ERROR] Unexpected error calling TPS2 webhook: {e}")
