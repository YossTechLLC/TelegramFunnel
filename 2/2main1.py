...
def fetch_closed_channel_id():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT tele_closed FROM test_channel WHERE tele_open = %s", (global_open_channel_id,))
        result = cur.fetchone()
        if result:
...
def decode_start():
    ...
    try:
        hash_part, _, sub = token.partition("_")
