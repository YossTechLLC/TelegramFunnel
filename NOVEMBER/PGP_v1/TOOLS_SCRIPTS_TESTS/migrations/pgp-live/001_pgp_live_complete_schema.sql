-- ============================================================================
-- PayGatePrime Complete Database Schema - PGP-LIVE Project
-- ============================================================================
-- Project: pgp-live
-- Database: telepaydb
-- Instance: pgp-live:us-central1:pgp-telepaypsql
-- Migration: 001_pgp_live - Initial Schema Creation (13 Tables)
-- Created: 2025-11-18
-- Services: All PGP_*_v1 services
--
-- IMPORTANT CHANGES FROM ORIGINAL:
-- ❌ EXCLUDED: donation_keypad_state (deprecated bot UI state)
-- ❌ EXCLUDED: user_conversation_state (deprecated conversation state)
-- ✅ INCLUDES: 13 core operational tables
--
-- Service Name Mapping (Old GC → New PGP_v1):
--   GCRegisterAPI-10-26 → PGP_SERVER_v1 (web portal)
--   GCWebhook1-10-26 → PGP_ORCHESTRATOR_v1 (payment orchestration)
--   GCWebhook2-10-26 → PGP_INVITE_v1 (telegram invites)
--   GCBroadcastScheduler-10-26 → PGP_BROADCAST_v1 (broadcast management)
--   GCAccumulator-10-26 → PGP_ACCUMULATOR_v1 (payment accumulation)
--   GCMicroBatchProcessor-10-26 → PGP_MICROBATCHPROCESSOR_v1 (batch processing)
--   GCSplit1/2-10-26 → PGP_SPLIT1_v1/PGP_SPLIT2_v1 (split payouts)
--   GCHostPay1/2/3-10-26 → PGP_HOSTPAY1_v1/PGP_HOSTPAY2_v1/PGP_HOSTPAY3_v1
--
-- ⚠️  WARNING: DO NOT RUN ON PRODUCTION WITHOUT BACKUP
-- ============================================================================
...
COMMENT ON TYPE currency_type IS 'PGP_v1: Supported cryptocurrency types';
...
COMMENT ON TYPE network_type IS 'PGP_v1: Supported blockchain networks';
...
COMMENT ON TYPE flow_type IS 'PGP_v1: ChangeNOW exchange flow types';
...
COMMENT ON TYPE type_type IS 'PGP_v1: ChangeNOW exchange direction types';
...
-- ============================================================================
-- STEP 6: Feature Tables (Broadcast Management)
-- ============================================================================
...
DO $$
BEGIN
    RAISE NOTICE '✅ PayGatePrime PGP-LIVE Schema Creation Complete';
    RAISE NOTICE '📊 Created 13 tables with indexes and constraints';
    RAISE NOTICE '🔐 Created 4 custom ENUM types';
    RAISE NOTICE '👤 Created legacy_system user for existing data';
    RAISE NOTICE '❌ EXCLUDED: donation_keypad_state (deprecated)';
    RAISE NOTICE '❌ EXCLUDED: user_conversation_state (deprecated)';
    RAISE NOTICE ' ';
    RAISE NOTICE 'Next steps:';
    RAISE NOTICE '1. Populate currency_to_network with 87 currency/network mappings';
    RAISE NOTICE '2. Verify schema with verification script';
    RAISE NOTICE '3. Update service code references from client_table to pgp_live_db';
END $$;
