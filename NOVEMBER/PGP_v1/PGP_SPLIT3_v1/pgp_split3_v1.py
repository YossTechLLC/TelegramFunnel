...
# Initialize managers
logger.info(f"🚀 [APP] Initializing PGP_SPLIT3_v1 ETH→Client Swapper Service")
...
def process_eth_client_swap():
    ...
    """
    try:
        logger.info(f"🎯 [ENDPOINT] ETH→Client swap request received (from PGP_SPLIT1_v1)")
        ...
        logger.info(f"🌐 [ENDPOINT] Creating ChangeNow transaction {swap_currency.upper()}→{payout_currency.upper()} (with retry)")
...
def process_eth_to_usdt_swap():
    ...
    """
    try:
        logger.info(f"🎯 [ENDPOINT] ETH→USDT swap request received (from PGP_ACCUMULATOR)")
        ...
        logger.info(f"💰 [ENDPOINT] ETH Amount: ${eth_amount} (USD equivalent)")
        logger.info(f"🎯 [ENDPOINT] Target: USDT on ETH network")
        logger.info(f"🏦 [ENDPOINT] USDT Wallet: {usdt_wallet_address}")
        ...
        logger.info(f"🌐 [ENDPOINT] Creating ChangeNow ETH→USDT transaction (with retry)")
        ...
        logger.debug(f"🆔 [ENDPOINT] ChangeNow API ID: {cn_api_id}")
        logger.info(f"🏦 [ENDPOINT] Payin address: {api_payin_address}")
        logger.info(f"💰 [ENDPOINT] From: ${api_from_amount} ETH (USD equivalent)")
        logger.info(f"💰 [ENDPOINT] To: ${api_to_amount} USDT")
        ...
        logger.info(f"✅ [ENDPOINT] Successfully enqueued response to PGP_ACCUMULATOR")
        logger.debug(f"🆔 [ENDPOINT] Task: {task_name}")
...
if __name__ == "__main__":
    logger.info(f"🚀 [APP] Starting PGP_SPLIT3_v1 on port 8080")
