...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "CMD_START":
        ...
        # Handle Database button press by starting the conversation properly
        # Remove the keyboard (edit message), then prompt for tele_open
        await query.edit_message_text(
            "Enter *tele_open* (≤14 chars integer):",
            parse_mode="Markdown"
        )
        # Save that user is in the conversation
...
# Only needed for /database command, not the button!
...
async def run_telegram_bot():
    ...
    database_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(main_menu_callback, pattern="^CMD_DATABASE$"),  # This makes Database button start the convo!
        ],
        states={
    ...
    # Menu callback for other buttons
